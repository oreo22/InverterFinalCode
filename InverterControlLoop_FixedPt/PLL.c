...
/*	coswave[c_index]= spll_obj->AC_input;
	...
	// Put in the phase shift: Phase angle (deg) = time delay  × frequency f × 360
...
//	}
    ...
    int sinIndex=(int)ceil(sinTheta*(1023)); //translating the theta into an index value
		spll_obj->sin[0]= sineSign*sineTable[sinIndex]; //calculating sin(theta)*1 or  sin(theta)*-1 dependent on the quadrant sign
	
		sineSign=1;
    float cosIndex=spll_obj->theta[0]+(QPI2); //changing the theta for cos and repeating the process
    if(cosIndex>(Q2PI)){ //Q1
