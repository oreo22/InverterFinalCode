...
void ADCTask(void)//void *pvParameters
{
 ...
 if(rmsFlag==1){// Polish RMS value 
		...
		dcMeas=(dcMeas*8.3253 ); //multiply by 8.5247 for the voltage divider and then divide by 1.414 
		UARTprintf("Vdc: %d \n",(int)dcMeas);
		...
		Vpcc.rms=avg_Vbus*scaling[scale_index]; 
		UARTprintf("Vbus: %d \n",(int)Vpcc.rms); 
		
		
		if(count>=250 && Vpcc.rms>=(dcMeas/1.414)){
			ma_avg=0; //turn off control signals and let the battery charge 
			//IntDisable(INT_PWM0_0);
			batCharge=1;
		}
		if(count>=250 && (Vpcc.rms > 3000 && Vpcc.rms <8000)){ //&&(Sinv.V.rms<(4.9) || Sinv.V.rms>(5.1))
			ctrlFlag=1;
		}
		if(batCharge==1 && Vpcc.rms>=(dcMeas/1.414)){
			batCharge=0;
		}
		if(ctrlFlag==0 && batCharge==0){
			ma_avg=(Vpcc.rms*1.414)/dcMeas;
			WattControl(&Pgrid,&Sinv);
		}
			...
			Sinv.P.rms=((Sinv.P.rms))*14.642*9.122;
			//UARTprintf("Vdc %d \n",(int) (dcMeas));
			Pgrid.rms=Pgrid.sum;
			Pgrid.rms=((Pgrid.rms))*scaling[scale_index]*9.122;
...
//			Sinv.V.rms/=1000;
//			Sinv.I.rms/=1000;
			Sinv.S = Sinv.V.rms*Sinv.I.rms; //can't keep it in ints cause it overflows since the max an int can store is 2,147,483,647, and squaring S can go up to (100e^12) 
			long long int  S_sqed=Sinv.S *Sinv.S;
			long long int P_sqed=Sinv.P.rms*Sinv.P.rms;
			if(Sinv.S >=Sinv.P.rms){
				Sinv.Q=sqrtInt(S_sqed-P_sqed);
			}
			//Vpcc.rms=((double)Vpcc.rms);
		rmsFlag=0;
		count++;
if(ctrlFlag==1){
